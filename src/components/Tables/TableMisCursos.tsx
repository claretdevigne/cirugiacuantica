...
import { getMisCursos, addCourse, deleteCourses, editCourses } from '@/app/lib/dbActions';
...
const fetchCourses = async () => {
  ...
  const response = await getMisCursos(user.name)
  return JSON.parse(response)
};

const TableOne = () => {
  const { isLoading, data: misCursos, refetch } = useQuery({queryKey: ["courses"], queryFn: fetchCourses});
  ...
  const filteredCourses = misCursos?.filter((course: Course) =>
